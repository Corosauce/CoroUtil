...
public class ParticleTexLeafColor extends ParticleTexFX {
	...
	public ParticleTexLeafColor(World worldIn, double posXIn, double posYIn,
		...
		if (state.getOwner() instanceof DoublePlantBlock && state.get(DoublePlantBlock.HALF) == EnumBlockHalf.UPPER) {
		    state = state.withProperty(DoublePlantBlock.VARIANT, worldIn.getBlockState(pos.down()).get(DoublePlantBlock.VARIANT));
		}
	...
	private final boolean hasColor(BlockState state) {
		return blockColorMap.containsKey(state.getOwner().delegate);
	}
