...
public abstract class GuiSlot
{
    ...
    public int slotSizeHalf = 80;
    
    public boolean drawGradients = true;

    public static final ResourceLocation resGUI = new ResourceLocation("/gui/background.png");
    ...
    {
        int var3 = this.width / 2 - slotSizeHalf;
        int var4 = this.width / 2 + slotSizeHalf;
    ...
    public void drawScreen(int par1, int par2, float par3)
    {
        ...
        //customizing fixes to set it to where it should be according to gradients
        int offsetFixX = -20;
        int lengthFixX = 34;
        int offsetFixYClipping = 10;
        ...
        if (Mouse.isButtonDown(0))
        {
            if (this.initialClickY == -1.0F)
            {
                ...
                {
                    int var8 = this.width / 2 - slotSizeHalf + left + offsetFixX;
                    var9 = this.width / 2 + slotSizeHalf + left + offsetFixX + lengthFixX;
        ...
        for (var11 = 0; var11 < var4; ++var11)
        {
            ...
            if (var19 <= this.bottom - offsetFixYClipping && var19 + var13 >= this.top + offsetFixYClipping)
            {
                if (this.showSelectionBox && this.isSelected(var11))
                {
                    var14 = this.width / 2 - slotSizeHalf + left + offsetFixX;
                    int var15 = this.width / 2 + slotSizeHalf + left + offsetFixX + lengthFixX;
        ...
        //background, top bottom gradients
        GL11.glDisable(GL11.GL_DEPTH_TEST);
        byte var20 = 16;
        //this.overlayBackground(this.left, this.top, 255, 255);
        ...
        if (drawGradients) {
	        var18.startDrawingQuads();
	        var18.setColorRGBA_I(0x000000, 0);
	        ...
	        var18.setColorRGBA_I(0x000000, 255);
    ...
    {
        return this.width / 2 + (slotSizeHalf + 14) + left;
    }
    ...
    protected void overlayBackground(int par1, int par2, int par3, int par4)
    {
        ...
        var5.addVertexWithUV((double)this.width, (double)par2, 0.0D, (double)((float)this.width / var6) + left, (double)((float)par2 / var6));
        var5.setColorRGBA_I(4210752, par3);
        var5.addVertexWithUV((double)this.width, (double)par1, 0.0D, (double)((float)this.width / var6) + left, (double)((float)par1 / var6));
    ...
    protected void drawContainerBackgroundFull(Tessellator tess)
    {
        ...
        tess.addVertexWithUV((double)mc.displayWidth, (double)bottom, 0.0D, (double)(mc.displayWidth / height), (double)((bottom + (int)amountScrolled) / height));
        tess.addVertexWithUV((double)mc.displayWidth, (double)top,    0.0D, (double)(mc.displayWidth / height), (double)((top    + (int)amountScrolled) / height));
