...
public class MovingBlock extends Entity implements IEntityAdditionalSpawnData
{
    ...
    public void onUpdate()
    {
    	...
    	/*if (world.isRemote) {
    		...
    		if (firstTick) {
    			...
    			//PacketDispatcher.sendPacketToAllInDimension(new Packet34EntityTeleport(getEntityId(), this.myEntitySize.multiplyBy32AndRound(posX), this.myEntitySize.multiplyBy32AndRound(posY), this.myEntitySize.multiplyBy32AndRound(posZ), (byte)0, (byte)0), world.provider.dimensionId);
    		}
    	}*/
        ...
        if (!world.isRemote) {
	    	...
	    	if (blockifyDelay != -1) {
		    	if (idCurPos != 0 && world.getBlockTileEntity(curX, curY, curZ) == null && isSolid(idCurPos)) {
		    		//final check to help verify its falling into solid ground
		    		if (world.getBlockId(curX, curY+1, curZ) != 0) {
    ...
    public void spawnParticles() {
    	/*for (int i = 0; i < 1; i++) {
    		...
    		EntityMeteorTrailFX particle = new EntityMeteorTrailFX(world,
    				posX,
    				posY,
