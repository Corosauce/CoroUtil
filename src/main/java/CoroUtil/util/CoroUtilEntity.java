...
public class CoroUtilEntity {
    ...
    public static PlayerEntity getClosestVulnerablePlayer(World world, double p_72846_1_, double p_72846_3_, double p_72846_5_, double p_72846_7_)
    {
        ...
        for (int i = 0; i < world.playerEntities.size(); ++i)
        {
            ...
            if (!entityplayer1.abilities.disableDamage && entityplayer1.isAlive())
            {
    ...
    public static boolean canSpawnMobOnGround(World world, int x, int y, int z) {
        ...
        if (CoroUtilBlock.isAir(block) || !block.canCreatureSpawn(state, world, pos, MobEntity.SpawnPlacementType.ON_EARTH)) {
            return false;
        }
        return true;
    }
    ...
    public static boolean canPathfindLongDist(CreatureEntity ent) {
        long lastPathTime = ent.getPersistentData().getLong(UtilEntityBuffs.dataEntityBuffed_LastTimePathfindLongDist);
    ...
    public static void updateLastTimeLongDistPathfinded(CreatureEntity ent) {
        ent.getPersistentData().putLong(UtilEntityBuffs.dataEntityBuffed_LastTimePathfindLongDist, ent.world.getGameTime() + (ent.getEntityId() % 20));
    }
}
