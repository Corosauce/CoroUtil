...
public class PathNavigateCustom
{
    ...
    private void pathFollow()
    {
        ...
        //if (MathHelper.floor(vec3.zCoord) > -17 && MathHelper.floor(vec3.zCoord) < -13) {
        	...
        	Block block = this.theEntity.world.getBlockState(new BlockPos(pp.x, pp.y, pp.z)).getOwner();
    ...
    private int getPathableYPos()
    {
        if (this.theEntity.isInWater() && this.canSwimOnSurface)
        {
            ...
            Block block = this.world.getBlockState(new BlockPos(MathHelper.floor(this.theEntity.posX), i, MathHelper.floor(this.theEntity.posZ))).getOwner();
            int k = 0;
            ...
            do
            {
                ...
                ++i;
                block = this.world.getBlockState(new BlockPos(MathHelper.floor(this.theEntity.posX), i, MathHelper.floor(this.theEntity.posZ))).getOwner();
                ++k;
            }
            while (k <= 16);
        ...
        else
        {
            for (int i2 = k1; i2 < k1 + par4; ++i2)
            {
                for (int j2 = l1; j2 < l1 + par6; ++j2)
                {
                    ...
                    {
                    	Block block = this.world.getBlockState(new BlockPos(i2, par2 - 1, j2)).getOwner();
    ...
    private boolean isPositionClear(int p_179692_1_, int p_179692_2_, int p_179692_3_, int p_179692_4_, int p_179692_5_, int p_179692_6_, Vec3d p_179692_7_, double p_179692_8_, double p_179692_10_)
    {
        for (BlockPos blockpos : BlockPos.getAllInBox(new BlockPos(p_179692_1_, p_179692_2_, p_179692_3_), new BlockPos(p_179692_1_ + p_179692_4_ - 1, p_179692_2_ + p_179692_5_ - 1, p_179692_3_ + p_179692_6_ - 1)))
        {
            ...
            {
                Block block = this.world.getBlockState(blockpos).getOwner();
