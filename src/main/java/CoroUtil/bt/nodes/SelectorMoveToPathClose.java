					...
					if ((entInt.getAIBTAgent().pathNav.noPath() && !lastAttemptFailed) || (lastPathTime + repathDelay < ent.world.getGameTime() && ent.world.getGameTime() % 20 == 0)) {
						...
						if (partialPathing) {
							if (lastPathTime % 5 == 0) {
		    			        ...
		    			        Block id = ent.world.getBlockState(new BlockPos(coordX, coordY, coordZ)).getOwner();
		    			        int tries = 0;
		    			        if (CoroUtilBlock.isAir(id)) {
		    			        	Block idUp = ent.world.getBlockState(new BlockPos(coordX, coordY+1, coordZ)).getOwner();
		    			        	Block idDown = ent.world.getBlockState(new BlockPos(coordX, coordY-1, coordZ)).getOwner();
