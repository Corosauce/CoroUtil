...
*/        
	...
	public void tickMovement() {
		...
		double speed = 0.2D;
		Block block = ent.world.getBlockState(new BlockPos(MathHelper.floor(ent.posX), (int)ent.getBoundingBox().minY, MathHelper.floor(ent.posZ))).getOwner();
		if (PFQueue.isFenceLike(block)) {
			...
			block = ent.world.getBlockState(new BlockPos(MathHelper.floor(ent.posX), (int)ent.getBoundingBox().minY-1, MathHelper.floor(ent.posZ))).getOwner();
			if (PFQueue.isFenceLike(block)) {
				Random rand = new Random();
				ent.motionX += rand.nextDouble()*speed - rand.nextDouble()*speed;
				ent.motionY = 0.2F;
