	...
	public void spawnInvasionMob(World world, PlayerEntity player, String spawn, DataActionMobSpawns spawns, Vec3d posVec) {
		...
		if (clazz != null) {
			...
			if (entL != null && entL instanceof CreatureEntity) {
				...
				ent.getPersistentData().putBoolean(UtilEntityBuffs.dataEntityWaveSpawned, true);
				UtilEntityBuffs.registerAndApplyCmods(ent, spawns.cmods, difficultyScale);

				ent.getPersistentData().putBoolean(UtilEntityBuffs.dataEntityInitialSpawn, true);
				addEntity0(player, ent);
				ent.getPersistentData().putBoolean(UtilEntityBuffs.dataEntityInitialSpawn, false);
