...
public class EventHandlerForge {
		...
		if (CoroUtilCompatibility.isHWInvasionsInstalled()) {
			if (!event.getEntity().world.isRemote) {
				if (event.getEntity() instanceof PlayerEntity) {
					...
					//event.getEntity().getPersistentData().putFloat(DynamicDifficulty.dataPlayerInvasionSkipBuff, 0);
				}
	...
	public void entityTick(LivingUpdateEvent event) {
						...
						if (id == Blocks.ORGANIC) {
							...
							if (bdp.walkedOnAmount > 5F) {
								...
								if (ent.world.getBlockState(new BlockPos(newX, newY+1, newZ)).getBlock() == Blocks.AIR) {
									ent.world.setBlockState(new BlockPos(newX, newY, newZ), Blocks.ORGANIC_PATH.getDefaultState());
								}
		...
		if (!ent.world.isRemote) {
			if ((ent.world.getGameTime() + ent.getEntityId()) % 20 == 0) {
				...
				if (ConfigCoroUtilAdvanced.removeInvasionAIWhenInvasionDone && ent.getPersistentData().getBoolean(UtilEntityBuffs.dataEntityBuffed)) {
					...
					if (ent instanceof MobEntity) {
						...
						it = entL.targetSelector.taskEntries.iterator();
						while (it.hasNext()) {
							GoalSelector.EntityAITaskEntry task = it.next();
							if (task.action instanceof IInvasionControlledTask) {
								if (((IInvasionControlledTask) task.action).shouldBeRemoved()) {
									//entL.targetSelector.removeGoal(task.action);
									task.action.resetTask();
									it.remove();
								}
							}
			...
			if (ent.getPersistentData().getCompound(UtilEntityBuffs.dataEntityBuffed_Data).getBoolean(UtilEntityBuffs.dataEntityBuffed_AI_Digging)) {
				...
				if (pushMobsAwayForMiners) {
					...
					if (!list.isEmpty())
					{
						...
						for (int l = 0; l < list.size(); ++l)
						{
							...
							CompoundNBT data2 = entityIn.getPersistentData().getCompound(UtilEntityBuffs.dataEntityBuffed_Data);

							//if config allows, push only buffed mobs, including wave spawned and extra enhanced ones
							boolean canPush = !ConfigCoroUtilAdvanced.minersPushAwayOnlyOtherBuffedMobs || ent.getPersistentData().getBoolean(UtilEntityBuffs.dataEntityBuffed);
				...
				if (waterFixLeapOut) {
					...
					if (ent.getPersistentData().getBoolean(nbtID)) {
						if (!ent.isInWater()) {
					...
					ent.getPersistentData().putBoolean(nbtID, ent.isInWater());
				}
			}
	...
	public void entityCreated(EntityJoinWorldEvent event) {
		...
		if (event.getEntity() instanceof CreatureEntity) {
			...
			if (ent.getPersistentData().getBoolean(UtilEntityBuffs.dataEntityBuffed) && !ent.getPersistentData().getBoolean(UtilEntityBuffs.dataEntityInitialSpawn)) {
	...
	public void playerCloneEvent(PlayerEvent.Clone event) {
		...
		CompoundNBT nbtOld = event.getOriginal().getPersistentData();
		CompoundNBT nbtNew = event.getEntityPlayer().getPersistentData();
