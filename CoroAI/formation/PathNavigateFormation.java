...
public class PathNavigateFormation
{
    ...
    public void onUpdateNavigation()
    {
        ...
        if (!this.noPath())
        {
            ...
            if (!this.noPath())
            {
                Vec3 vec3 = getVectorFromIndex(currentPath.getCurrentPathIndex());

                if (vec3 != null)
                {
                    moveHelper.setMoveTo(vec3.xCoord, vec3.yCoord, vec3.zCoord, this.speed);
                }
            }
        }
    }
    ...
    private void pathFollow()
    {
        ...
        {
            if (vec3.squareDistanceTo(getVectorFromIndex(k)) < (double)f)
            {
                this.currentPath.setCurrentPathIndex(k + 1);
            }
        }

        k = MathHelper.ceiling_float_int(width);
        int l = (int)height + 1;
        int i1 = k;

        for (int j1 = i - 1; j1 >= this.currentPath.getCurrentPathIndex(); --j1)
        {
            if (this.isDirectPathBetweenPoints(vec3, getVectorFromIndex(j1), k, l, i1))
            {
    ...
    {
        return pos;//this.worldObj.getWorldVec3Pool().getVecFromPool(this.theEntity.posX, (double)this.getPathableYPos(), this.theEntity.posZ);
    }
    ...
    {
        return true;//this.theEntity.onGround || this.canSwim && this.isInFluid();
    }
    ...
    {
        return false;//this.theEntity.isInWater() || this.theEntity.handleLavaMovement();
    }
