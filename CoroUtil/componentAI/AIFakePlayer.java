...
public class AIFakePlayer {
		    		...
		    		try {
		    			...
		    			if (ai.ent.worldObj.playerEntities.size() > 0)
		    	        {
		    	            if (ai.ent.worldObj.playerEntities.get(0) instanceof EntityPlayerMP)
		    	            {
			            	    ...
			            	    OldUtil.playerToCompAILookup.put(fakePlayer.username, ai.entInt);
		    	            }
		    	        }
		    	        else
		    	        {
				...
				if (itemToUse.getItem() instanceof ItemTropicalFishingRod) {
					...
					if (forceHoldAndRelease || itemToUse.getItem() instanceof ItemBow || itemToUse.getItem() instanceof ItemFood) {
						...
						if (oldMethod) {
							...
							OldUtil.setPrivateValueSRGMCP(EntityPlayer.class, fakePlayer, "field_71072_f", "itemInUseCount", inUseCount);
	...
	public void setPrjOwner(int offset) {
    	...
    	if (ent instanceof EntityThrowable) {
    		...
    		OldUtil.setPrivateValueSRGMCP(EntityThrowable.class, ((EntityThrowable) ent), OldUtil.refl_thrower_obf, OldUtil.refl_thrower_mcp, ai.ent);
	...
	public c_EntityPlayerMPExt newFakePlayer(World world)
    {
        ...
        if (OldUtil.chunkExists(world, (int)(ai.ent.posX / 16), (int)(ai.ent.posZ / 16))) {
    ...
    public void cleanup() {
		if (fakePlayer != null) OldUtil.playerToCompAILookup.remove(fakePlayer.username);
    	fakePlayer = null;
    	inventory = null;
    }
